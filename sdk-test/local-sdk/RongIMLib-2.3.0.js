...
(function (RongIMLib) {
    var ServerDataProvider = (function () {
        ...
        ServerDataProvider.prototype.connect = function (token, callback) {
            var hasChannel = (RongIMLib.Bridge._client && RongIMLib.Bridge._client.channel);
            if (hasChannel) {
                var state = RongIMLib.Bridge._client.channel.connectionStatus;
                var Status = RongIMLib.ConnectionStatus;
                var isConnected = (state == Status.CONNECTED);
                var isConnecting = (state == Status.CONNECTING);
        ...
        */
        ServerDataProvider.prototype.reconnect = function (callback, config) {
            ...
            var state = this.getCurrentConnectionStatus();
            var isConnected = (state == RongIMLib.ConnectionStatus.CONNECTED)
            if (!isConnected) {
                config = config || {};
