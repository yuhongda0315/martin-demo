    ...
    //feature detection 功能判断，DOM加载完成时执行此方法
        , func = function () {
            ...
            if ("WebSocket" in global && "ArrayBuffer" in global && WebSocket.prototype.CLOSED === 3 && !global.WEB_SOCKET_FORCE_FLASH && !global.WEB_XHR_POLLING) {
                //加载protobuf
                var RongOpts = global.RongOpts || {};
                script.src = RongOpts.PB_URL || 'http://res.websdk.rongcloud.cn/protobuf.min.0.5.js';
                //是否支持flash widget
            ...
            script.onload = function(){
                var connect = RongIMClient.connect;
                RongIMClient.connect(connect.token, connect.callback);
            };
...
//channel object constructor 最顶层的通道对象，内部封装了io对象
    ...
    function pottingProfile(item, publicServiceList) {
        var profile = new RongIMClient.PublicServiceProfile(), temp = JSON.parse(item.extra);
        profile.setIsGlobal(temp.isGlobal);
        profile.setIntroduction(temp.introduction);
        profile.setMenu(temp.menu);
        profile.setHasFollowed(temp.follow);
        profile.setPublicServiceId(item.mpid);
        profile.setName(item.name);
        profile.setPortraitUri(item.portraitUrl);
        profile.setConversationType(item.type == 'mc' ? RongIMClient.ConversationType.APP_PUBLIC_SERVICE : RongIMClient.ConversationType.PUBLIC_SERVICE);
        publicServiceList.push(profile);
    }
    ...
    function callbackMapping(entity, tag) {
        switch (tag) {
            ...
            case "PullMpOutput":
                ...
                if (entity.info) {
                    io.util.forEach(entity.info, function (item) {
                        setTimeout(pottingProfile(item, publicServiceList), 100);
                    });
                }
    ...
    //topic白名单
    var _topic = ["invtDiz", "crDiz", "qnUrl", "userInf", "dizInf", "userInf", "joinGrp", "quitDiz", "exitGrp", "evctDiz", ["chatMsg", "pcMsgP", "pdMsgP", "pgMsgP", "ppMsgP","","","pmcMsgN","pmpMsgN"], "pdOpen", "rename", "uGcmpr", "qnTkn", 'destroyChrm', 'createChrm', 'exitChrm', 'queryChrm', 'joinChrm', "pGrps", "addBlack", "rmBlack", "getBlack", "blackStat", "addRelation", 'qryRelation', 'delRelation', 'pullMp', 'schMp'];
    //桥连类，用于连接外部操作和内部操作
    ...
    //存储公众帐号列表的类
    var publicServiceListMap = function () {
        this.add = function (p) {
            var isAdd = true, me = this;
            for (var i = 0, len = this.length; i < len; i++) {
                if (me[i].getConversationType() == p.getConversationType() && p.getPublicServiceId() == me[i].getPublicServiceId()) {
                    me.unshift(me.splice(i, 1)[0]);
                    isAdd = false;
                    break;
                }
            }
        ...
        this.get = function (conver, pid) {
            for (var i = 0, len = this.length; i < len; i++) {
                if (this[i].getConversationType() == conver && pid == this[i].getPublicServiceId()) {
                    return this[i];
                }
            }
        }
        this.remove = function (conver, pid) {
            for (var i = 0, len = this.length; i < len; i++) {
                if (this[i].getConversationType() == conver && pid == this[i].getPublicServiceId()) {
                    this.splice(i, 1);
                    break;
    ...
    publicServiceListMap.prototype = [];
    //存储会话列表的类
    var _func = function () {
    ...
    function RongIMClient(_appkey) {
        ...
        //公众帐号列表
            _PublicServiceList = new publicServiceListMap(),
        //本地临时会话存储，用于存储草稿
        ...
        pottingPublicSearchType = function (bussinessType, searchType) {
            //    businessType 0-all 1-mp 2-mc
            //    searchType 0-exact 1-fuzzy
            var bits = 0;
            if (bussinessType == 0) {
                bits |= 3;
                if (searchType == 0)
        ...
        this.syncPublicServiceList = function (callback) {
            var modules = new Modules.PullMpInput(), self = this;
            modules.setTime(0);
            modules.setMpid("");
            a.queryMsg(28, io.util.arrayFrom(modules.toArrayBuffer()), bridge._client.userId, {
                onSuccess: function (data) {
                    _PublicServiceList.length = 0;
                    io.util.forEach(data,function(item){
                        _PublicServiceList.push(item);
                    });
                },
                onError: function (e) {
                    callback.onError(e);
        ...
        this.getPublicServiceList = function () {
           return _PublicServiceList;
        };
        this.getPublicServiceProfile = function (conver, pid) {
            check(["number|object", "string", "object"]);
            return this.getPublicServiceList().get(conver,pid);
        };
        this.searchPublicService = function (searchType,keywords,callback) {
            check(["number","string","object"]);
            var modules = new Modules.SearchMpInput();
            modules.setType(pottingPublicSearchType(0, searchType));
            modules.setId(keywords);
            a.queryMsg(29,io.util.arrayFrom(modules.toArrayBuffer()), bridge._client.userId,callback, "SearchMpOutput");
        };
        this.searchPublicServiceByType = function (conver,searchType,keywords,callback) {
            check(["number","string","object"]);
            var modules = new Modules.SearchMpInput(),type = conver == RongIMClient.ConversationType.APP_PUBLIC_SERVICE ? 2 : 1;
            modules.setType(pottingPublicSearchType(type, searchType));
            modules.setId(keywords);
            a.queryMsg(29,io.util.arrayFrom(modules.toArrayBuffer()), bridge._client.userId,callback, "SearchMpOutput");
        };
        this.subscribePublicService = function (conver,pid,callabck) {
            check(["number","string","object"]);
            var modules = new Modules.MPFollowInput(),me = this,follow = conver == RongIMClient.ConversationType.APP_PUBLIC_SERVICE ? "mcFollow" : "mpFollow";
            modules.setId(pid);
            a.queryMsg(follow,io.util.arrayFrom(modules.toArrayBuffer()),bridge._client.userId,{
                onSuccess:function(){
                    me.syncPublicServiceList({
                        onSuccess: function(){},
                        onError: function(){}
                    });
                    callabck.onSuccess();
        ...
        this.unsubscribePublicService = function (conver,pid,callabck) {
            check(["number","string","object"]);
            var modules = new Modules.MPFollowInput(),me = this,follow = conver == RongIMClient.ConversationType.APP_PUBLIC_SERVICE ? "mcUnFollow" : "mpUnFollow";
            modules.setId(pid);
            a.queryMsg(follow,io.util.arrayFrom(modules.toArrayBuffer()),bridge._client.userId,{
                onSuccess:function(){
                    _PublicServiceList.remove(conver,pid);
                    callabck.onSuccess();
                },
                onError:function(){
                    callback.onError();
    ...
    RongIMClient.TypingStatusMessage = function (c) {
        ...
        this.setTypingContentType = function(a){
            this.setContent(a, "typingContentType");
        };
        this.getTypingContentType = function(a){
            return this.getDetail().typingContentType;
        };
    ...
    RongIMClient.TypingStatusMessage.obtain = function (typingContentType, data) {
        return new RongIMClient.TypingStatusMessage({
    ...
    RongIMClient.PublicServiceProfile = function () {
        var c, i, m, n, p, pid, h, isGlobal;
        ...
        this.getPublicServiceId = function () {
            return pid;
        };
        this.getHasFollowed = function () {
            return h;
        };
        this.getIsGlobal = function () {
            return isGlobal;
        };

        this.setConversationType = function (cover) {
            c = cover;
        };
        this.setIntroduction = function (introduc) {
            i = introduc;
        };
        ...
        this.setPortraitUri = function (portraitUri) {
            p = portraitUri;
        };
        this.setPublicServiceId = function (pubId) {
            pid = pubId;
        };
        this.setHasFollowed = function (hF) {
            h = hF;
        };
        this.setIsGlobal = function (i) {
            isGlobal = i;
        };
    };
    RongIMClient.PublicServiceMenuItem = function (i, n, t, s, u) {
        var i = i, n = n, t = t, s = s, u = u;
        this.getId = function () {
            return i;
        };
        this.getName = function () {
        ...
        this.setSunMenuItems = function (s) {
            s = s;
        };
...
//create enum object 枚举对象，详情可参考API文档
    var _enum = {
        ...
        'ConversationType': ['CHATROOM', 'CUSTOMER_SERVICE', 'DISCUSSION', 'GROUP', 'PRIVATE', 'SYSTEM', 'N', 'APP_PUBLIC_SERVICE', 'PUBLIC_SERVICE'],
        ...
        'MessageType': ['TypingStatusMessage', 'DiscussionNotificationMessage', 'TextMessage', 'ImageMessage', 'VoiceMessage', 'RichContentMessage', 'HandshakeMessage', 'UnknownMessage', 'SuspendMessage', 'LocationMessage', 'InformationNotificationMessage', 'ContactNotificationMessage', 'ProfileNotificationMessage', 'CommandNotificationMessage'],
        'SendErrorStatus': {
