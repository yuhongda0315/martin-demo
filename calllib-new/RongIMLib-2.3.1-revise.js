...
date: 2018-07-13
auth: yangchuan
update: Web 接收到撤回消息指令 RecallCommandMessage 修改 localStorage 未读数

TODO: 会话列表增加 免打扰设置属性 isBlocked 
      C++ 在 getTotalUnreadCount 接口增加 第二个参数 boolean 类型 false 表示获取未设置免打扰的会话未读总数
...
date: 2018-09-14
auth: yuhongda
update: 音视频相关消息类型增加 mode、subInfo
参考连接：http://gitbook.rongcloud.net/web/imsdk/rtc/rtc-dispatcher.html
*/
...
(function (RongIMLib) {
    var RongIMClient = (function () {
        ...
        RongIMClient.init = function (appKey, dataAccessProvider, options, callback) {
            ...
            RongIMClient.MessageParams["SubscribeModifyMessage"] = { objectName: "RC:MSubscribe", msgTag: new RongIMLib.MessageTag(false, false) };
            ...
            RongIMClient.MessageType = {
                ...
                SubscribeModifyMessage: "SubscribeModifyMessage",
...
(function (RongIMLib) {
    ...
    var MessageHandler = (function () {
        ...
        MessageHandler.prototype.onReceived = function (msg, pubAckItem, offlineMsg, leftCount, isSync) {
            ...
            if (isPersited) {
                RongIMLib.RongIMClient._dataAccessProvider.getConversation(message.conversationType, message.targetId, {
                    onSuccess: function (con) {
                        ...
                        var isRecall = message.messageType === 'RecallCommandMessage';
                        if (isRecall) {
                            con.unreadMessageCount && con.unreadMessageCount--;
                            if (RongIMLib.RongUtil.supportLocalStorage()) {
                                var count = +RongIMLib.RongIMClient._storageProvider.getItem("cu" + Bridge._client.userId + con.conversationType + con.targetId); // 与本地存储会话合并
                                count && count--;
                                RongIMLib.RongIMClient._storageProvider.setItem("cu" + Bridge._client.userId + con.conversationType + message.targetId, Number(count));
                            }
                        }
                        ...
                        if (!isRecall || (isRecall && con.latestMessage.messageUId == message.content.messageUId)) {
            ...
            var that = this;
            if (RongIMLib.RongIMClient._voipProvider && ['AcceptMessage', 'RingingMessage', 'HungupMessage', 'InviteMessage', 'MediaModifyMessage', 'MemberModifyMessage', 'SubscribeModifyMessage'].indexOf(message.messageType) > -1) {
                setTimeout(function () {
...
var typeMapping = {
    ...
    "RC:MSubscribe": "SubscribeModifyMessage",
...
(function (RongIMLib) {
    ...
    var SubscribeModifyMessage = (function () {
        function SubscribeModifyMessage(message) {
            this.messageName = "SubscribeModifyMessage";
            this.callId = message.callId;
            this.mode = message.mode;
            this.subInfo = message.subInfo;
        }
        SubscribeModifyMessage.prototype.encode = function () {
            return JSON.stringify(RongIMLib.ModelUtil.modelClone(this));
        };
        return SubscribeModifyMessage;
    })();
    RongIMLib.SubscribeModifyMessage = SubscribeModifyMessage;
    ...
    var VCDataProvider = (function () {
        ...
        VCDataProvider.prototype.setOnReceiveMessageListener = function (listener) {
            ...
            me.addon && me.addon.setOnReceiveMessageListener(function (result, leftCount) {
                ...
                setTimeout(function () {
                    var voipMsgTypes = ['AcceptMessage', 'RingingMessage', 'HungupMessage', 'InviteMessage', 'MediaModifyMessage', 'MemberModifyMessage', 'SubscribeModifyMessage'];
