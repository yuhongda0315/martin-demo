...
var RongIMClient = (function () {
     ...
     */
    RongIMClient.prototype.getHistoryMessages = function (conversationType, targetId, timestamp, count, objectName, direction, callback) {
        RongIMLib.CheckParam.getInstance().check(["number", "string", "number|null|global|object", "number", "string", "boolean|null|global|object", "object"], "getHistoryMessages");
        if (count > 20) {
        ...
        RongIMClient._dataAccessProvider.getHistoryMessages(conversationType, targetId, timestamp, count, objectName, direction, callback);
    };
    /**
    ...
    RongIMClient.prototype.sortConversationList = function (conversationList) {
        var convers = [];
        for (var k = conversationList.length-1; k >= 0; k--){
            if (!conversationList[k]) {
                continue;
            }
            if (conversationList[k].isTop) {
                convers.push(conversationList[k]);
                conversationList.splice(k, 1);
                continue;
            }
        }
...
var VCDataProvider = (function () {
    ...
    VCDataProvider.prototype.registerMessageType = function (messageType, objectName, messageTag, messageContent) {
        ...
        var regMsg = RongIMLib.ModelUtil.modleCreate(messageContent, messageType);
        regMsg.messageName = messageType;
        RongIMLib.RongIMClient.RegisterMessage[messageType] = regMsg;
        registerMessageTypeMapping[objectName] = messageType;
    ...
    VCDataProvider.prototype.getHistoryMessages = function (conversationType, targetId, timestamp, count, objectName, direction, callback) {
    ...
    /**
        ...
        params.targetId
        params.messageId // 被撤回消息的 messageId
        params.sentTime
        params.messageUId // 被撤回消息的 messageUId
        params.sendUserId
    ...
    VCDataProvider.prototype.sendRecallMessage = function (content, sendMessageCallback) {
        ...
        this.addon.recallMessage("RC:RcCmd", data, content.push||"",
        function() {
            content.objectName = 'RC:RcCmd';
            content.messageName = '';
            var msg = me.buildMessage(data);
            sendMessageCallback.onSuccess(msg);
            data = JSON.stringify(content);
            me.addon.setMessageContent(content.messageId, data, content.objectName);
        },
        function(errorCode) {
